...
- **Training Data**: LemGendizedFilmRestorer
...
```mermaid
graph TD
    Input[RGB Input 256x256] --> Backbone[UniversalFilmRestorer]
...
model_path = "film_restorer_latest.pth"
ckpt = torch.load(model_path, map_location=device, weights_only=False)
state = ckpt.get('model_state', ckpt) if isinstance(ckpt, dict) else ckpt

# 3. Initialization
from models.factory import create_model
model = create_model("film_restorer").to(device)
...
> [!TIP]
> **Implementation Guide**: For high-performance deployment including ONNX (FP32/FP16) and standalone PyTorch snippets, refer to the **[film_restorer_usage.ipynb](film_restorer_usage.ipynb)** notebook in this directory.
...
- **Baseline Achievement**: **PSNR**: 21.989061317687472 | **SSIM**: 0.5813854336738586 | **LPIPS**: 0.5340118357549013
